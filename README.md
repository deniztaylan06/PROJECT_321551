...
High-risk departments may be monitored more intensively, which can reduce missingness and create systematic differences in data completeness between risk groups. As a result, part of the predictive signal could arise from missingness patterns rather than underlying operational risk drivers.


To mitigate this risk, we (i) explicitly analyze missingness by feature and by risk group and (ii) include missingness indicators in the modeling pipeline. Nevertheless, surveillance bias cannot be fully eliminated without richer metadata on monitoring intensity or a redesign of the data collection process. For this reason, model outputs should be interpreted as decision-support signals rather than causal evidence.
